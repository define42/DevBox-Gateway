...
|---|---|---|
| Generate and maintain an audit log | Gateway writes structured JSON audit logs. SauronAgent collects guest events, spools them, and forwards them to the collector. [Storage implementation](internal/sauron/sauron.go#L110) | **Implemented; operational verification needed.** Collection must be enabled, storage persistent, and disk-full/outage behaviour tested. Gateway-integrated collection defaults to disabled. |
