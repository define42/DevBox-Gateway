...
|---|---|---|
| Generate and maintain an audit log | DevBox-Gateway emits structured JSON application audits to HEC when configured, otherwise to a local file, and always starts its guest-event collector on AF_VSOCK port 9000. SauronAgent collects guest events, spools them, and forwards them to the collector. [Storage implementation](internal/sauron/sauron.go#L110) | **Conditional; operational verification needed.** HEC-only application logging can lose events; see [delivery boundaries](#delivery-and-compliance-boundaries). Guest agents must be installed and running, storage persistent, and disk-full/outage behaviour tested. New VMs receive a vsock device; existing VMs without one are not automatically migrated. |
...
- **Application audits:** HEC is the sole output when configured and uses a best-effort in-memory queue of up to 10,000 events. Retryable failures are retried with backoff, but queue overflow, process termination, an exhausted shutdown flush or non-retryable responses can lose audit events. There is no application disk spool, local file fallback or replay of old audit files. HEC-only logging cannot guarantee retention of every event, so generating the listed events alone does not establish the requirement to maintain a complete audit log. See [application delivery](internal/audit/hec.go).
...
- Address the application HEC queue's delivery gaps before claiming every required event is retained; HEC-only mode provides no local recovery copy.
