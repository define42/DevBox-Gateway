...
func TestDashboardJavaScriptSupportsAdminLifecycleInventory(t *testing.T) {
	...
	for description, fragment := range map[string]string{
		...
		"server usage percentage":      `Math.round((usedBytes / totalBytes) * 100)`,
		"yellow usage boundary":        `const SERVER_USAGE_YELLOW_MIN_PERCENT = 60`,
		"red usage boundary":           `const SERVER_USAGE_RED_ABOVE_PERCENT = 80`,
		"green usage threshold":        `let colorClass = "text-bg-success"`,
		"red usage threshold":          `usedPercent > SERVER_USAGE_RED_ABOVE_PERCENT`,
		"yellow usage threshold":       `usedPercent >= SERVER_USAGE_YELLOW_MIN_PERCENT`,
		"shared server usage label":    `${label}: ${usedPercent}% · ${formatServerCapacityGB(usedBytes)} used / ${formatServerCapacityGB(totalBytes)} total`,
