...
func TestDashboardJavaScriptSupportsAdminLifecycleInventory(t *testing.T) {
	...
	for description, fragment := range map[string]string{
		...
		"server memory percentage":     `Math.round((usedBytes / totalBytes) * 100)`,
		"yellow memory boundary":       `const MEMORY_YELLOW_MIN_PERCENT = 60`,
		"red memory boundary":          `const MEMORY_RED_ABOVE_PERCENT = 80`,
		"green memory threshold":       `let colorClass = "text-bg-success"`,
		"red memory threshold":         `usedPercent > MEMORY_RED_ABOVE_PERCENT`,
		"yellow memory threshold":      `usedPercent >= MEMORY_YELLOW_MIN_PERCENT`,
		"server memory label":          `Memory: ${usedPercent}% · ${formatServerMemoryGB(usedBytes)} used / ${formatServerMemoryGB(totalBytes)} total`,
