...
function bootstrap() {
  ...
  `;
    ...
    const baseImagesButton = root.querySelector("#base-images-button");
    ...
    const serverDiskIOIndicator = root.querySelector("#server-disk-io-indicator");
    const serverCPUIndicator = root.querySelector("#server-cpu-indicator");
    ...
    const baseImagesModal = root.querySelector("#base-images-modal");
    const baseImagesBackdrop = root.querySelector("#base-images-backdrop");
    const baseImagesClose = root.querySelector("#base-images-close");
    const baseImagesError = root.querySelector("#base-images-error");
    const baseImagesMessage = root.querySelector("#base-images-message");
    const baseImageUploadForm = root.querySelector("#base-image-upload-form");
    const baseImageFile = root.querySelector("#base-image-file");
    const baseImageUploadButton = root.querySelector("#base-image-upload-button");
    const baseImageUploadLimit = root.querySelector("#base-image-upload-limit");
    const baseImageAvailableStorage = root.querySelector("#base-image-available-storage");
    const baseImageUploadProgress = root.querySelector("#base-image-upload-progress");
    const baseImageUploadSpinner = root.querySelector("#base-image-upload-spinner");
    const baseImageUploadLabel = root.querySelector("#base-image-upload-label");
    const baseImageUploadValue = root.querySelector("#base-image-upload-value");
    const baseImageUploadTrack = root.querySelector("#base-image-upload-track");
    const baseImageUploadBar = root.querySelector("#base-image-upload-bar");
    const baseImagesList = root.querySelector("#base-images-list");
    if (!pageTitle ||
        !pageSubtitle ||
        !adminViewLink ||
        !dashboardViewLink ||
    ...
    function renderServerUsagePercent(indicator, iconClass, label, usagePercent, details = "") {
    ...
    function renderServerUsage(indicator, iconClass, label, usage) {
        ...
        renderServerUsagePercent(indicator, iconClass, label, (usedBytes / totalBytes) * 100, ` · ${formatServerCapacityGB(usedBytes)} used / ${formatServerCapacityGB(totalBytes)} total`);
    }
    ...
    function renderServerDiskIO(diskIO) {
        if (!adminView) {
            return;
        }
        const usagePercent = diskIO ? diskIO.usagePercent : null;
        const readBytesPerSecond = diskIO === null || diskIO === void 0 ? void 0 : diskIO.readBytesPerSecond;
        const writeBytesPerSecond = diskIO === null || diskIO === void 0 ? void 0 : diskIO.writeBytesPerSecond;
        if (typeof readBytesPerSecond !== "number" ||
        ...
        renderServerUsagePercent(serverDiskIOIndicatorEl, "bi-arrow-down-up", "Disk I/O", usagePercent, ` · R ${formatBytesPerSecond(readBytesPerSecond)} · W ${formatBytesPerSecond(writeBytesPerSecond)}`);
    }
    function renderServerCPU(cpu) {
        if (adminView) {
    ...
    function renderBaseImageManager() {
        ...
        for (const name of manager.images) {
            ...
            setIconLabel(deleteButton, manager.deleting === name ? "bi-hourglass-split" : "bi-trash", manager.deleting === name ? "Deleting..." : "Delete");
    ...
    function closeBaseImageManager() {
    ...
    async function loadManagedBaseImages() {
        const manager = state.baseImageManager;
        if (manager.loading) {
            return;
        }
        ...
        renderBaseImageManager();
        try {
            const result = await requestJSON(ADMIN_BASE_IMAGES_PATH);
            if (!result) {
                return;
            }
            ...
            manager.images = Array.isArray(result.data.baseImages)
                ? result.data.baseImages.filter((name) => typeof name === "string")
                : [];
    ...
    function requestBaseImageUpload(file) {
    ...
    async function uploadManagedBaseImage(file) {
        var _a;
        const manager = state.baseImageManager;
        if (manager.busy) {
            return;
        ...
        try {
            ...
            if (!result.ok || !result.data || result.data.ok !== true) {
                manager.error = result.error || ((_a = result.data) === null || _a === void 0 ? void 0 : _a.error) || "Base image upload failed.";
                return;
            }
    ...
    async function deleteManagedBaseImage(name) {
        var _a;
        const manager = state.baseImageManager;
        if (manager.busy) {
            return;
        ...
        try {
            ...
            const result = await requestJSON(ADMIN_BASE_IMAGES_DELETE_PATH, {
                method: "POST",
            ...
            if (!result.ok || !result.data || result.data.ok !== true) {
                manager.error = result.error || ((_a = result.data) === null || _a === void 0 ? void 0 : _a.error) || "Base image deletion failed.";
                return;
            }
    ...
    baseImageUploadFormEl.addEventListener("submit", (event) => {
        ...
        const file = (_a = baseImageFileEl.files) === null || _a === void 0 ? void 0 : _a.item(0);
        if (!file) {
